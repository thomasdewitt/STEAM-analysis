#!/usr/bin/env python3
"""Outer-scale comparison figures: STEAM 96 km vs STEAM L/4 = ls=10 m const.
...
Same layout family as make_deltas.py. Two figures:
  figs/profiles_spheroscale_const.png — raw profiles: host (solid), STEAM 96 km (dashed),
    STEAM ls=10 m const (dotted), per model color.
  figs/deltas_spheroscale_const.png — STEAM-minus-host deltas for both outer scales on
    the same inter-LES pairwise spread band (96 km thin, ls=10 m const thick).
Requires stats/steam_ls10_* and steam_ls1_* from run_steam_96.py / run_steam_ls10.py.
"""
...
steam96 = {m: snapshot_mean(f"steam_ls10_{m}") for m in models}
steamLs10 = {m: snapshot_mean(f"steam_ls1_{m}") for m in models}
models = [m for m in models
          if host[m] is not None and steam96[m] is not None
          and steamLs10[m] is not None]
print("models:", models)
...
for ax, (key, _, label) in zip(axes, QUANTITIES):
    for m, c in zip(models, colors):
        ...
        ax.plot(steamLs10[m][key], Z / 1000, color=c, lw=1.0, ls=":")
...
axes[2].legend(fontsize=6.5, loc="upper right",
               title="solid host / dash 10 m / dot 1 m")
fig.suptitle("Spheroscale at 96 km: constant 10 m vs constant 1 m (same seeds, frozen config)",
             fontsize=10)
fig.tight_layout()
(HERE / "figs").mkdir(exist_ok=True)
fig.savefig(HERE / "figs" / "profiles_spheroscale_const.png")
plt.close(fig)
...
for ax, (key, _, label) in zip(axes, QUANTITIES):
    ...
    for m, c in zip(models, colors):
        ax.plot(steam96[m][key] - host[m][key], Z / 1000, color=c, lw=0.8,
                alpha=0.6)
        ax.plot(steamLs10[m][key] - host[m][key], Z / 1000, color=c, lw=1.6,
                label=m)
...
axes[0].set_ylabel("z [km]")
axes[2].legend(fontsize=6.5, loc="upper right",
               title="thin 10 m / thick 1 m")
fig.suptitle("STEAM $-$ host LES: spheroscale constant 10 m vs 1 m", fontsize=10)
fig.tight_layout()
fig.savefig(HERE / "figs" / "deltas_spheroscale_const.png")
plt.close(fig)

print("wrote figs/profiles_spheroscale_const.png, figs/deltas_spheroscale_const.png")
