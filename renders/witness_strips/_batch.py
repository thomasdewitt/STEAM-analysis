"""Drive witness renders for every discovered STEAM strip subgroup.

Camera sits at the horizontal centre of each strip (x_mid, y_mid) and
shoots along the strip's long axis. Two views per strip/dataset combo:
  up_along   — just above the ocean surface, tilted up at the cloud layer.
  down_along — just above the domain top, tilted down at the clouds.
"""
...
RENDER_DIR = REPO / 'renders' / 'witness_strips'
NC_DIR = REPO / 'STEAM' / 'data'

IMG_SIZE = (1600, 1200)
STRIP_GROUP_CANDIDATES = ('strip_center', 'strip_edge')

# Pitch away from horizontal for the two views (degrees).
TILT_DEG = 8.0

# name, sign along strip long axis (+x / -x), z placement
VIEWS = [
    ('up_along',   +1, 'surface'),
    ('down_along', -1, 'above'),
]
...
def discover_strips():
    ...
    for nc_path in sorted(NC_DIR.glob('nested_refine_seed*.nc')):
        ...
        try:
            ...
            ds.close()
        for sub in STRIP_GROUP_CANDIDATES:
            if sub in present:
                strips.append((nc_path, f'refinements/{sub}', f'{sub}_seed{seed}'))
    return strips


def load_strip_level(nc_path: Path, group: str, ext_mult: float) -> NestedLevel:
...
def strip_camera_basis(sign_x, z_policy, bmin, bmax, tilt_deg):
    cam_x = 0.5 * (bmin[0] + bmax[0])
    cam_y = 0.5 * (bmin[1] + bmax[1])
    if z_policy == 'surface':
        cam_z = bmin[2] + 0.005 * (bmax[2] - bmin[2])
        tilt_sign = +1.0
    elif z_policy == 'above':
        cam_z = bmax[2] * 1.1
        tilt_sign = -1.0
    ...
    forward = np.array([sign_x * np.cos(t), 0.0, tilt_sign * np.sin(t)],
                       dtype=np.float64)
...
def main() -> None:
    ...
    strips = discover_strips()
    print(f"Discovered {len(strips)} strip groups:")
    for nc_path, group, tag in strips:
    ...
    for nc_path, group, tag in strips:
        ...
        t0 = time.perf_counter()
        level = load_strip_level(nc_path, group, ext_mult)
        print(f"  load + sigma: {time.perf_counter() - t0:.1f}s  "
              f"grid={level.sigma.shape}")
        ocean_z = (ocean_cfg['height'] + 1.0) * 0.5 * level.bmax[2]

        for view, sign_x, z_policy in pending:
            out = RENDER_DIR / f'witness_{tag}_{view}.png'
            cam, fwd, right, up = strip_camera_basis(
                sign_x, z_policy, level.bmin, level.bmax, TILT_DEG)
            t0 = time.perf_counter()
            image = render_nested(
                [level],
