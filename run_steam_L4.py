...
(= 6144 km domain length / 4, vs the production 96 km). For the C17
outer-scale ruling: profile figs comparing the two. Writes
runs/steam_L4_<model>_snap<i>.nc and stats/steam_L4_<model>_snap<i>.npz.
Skips snapshots whose stats file already exists (restartable).

Usage: python run_steam_L4.py [model ...]   (default: all with extracted stats)
"""
...
PROFILE_DZ = 50.0
OUTER_SCALE = 1536000.0  # 6144 km (longer domain dimension) / 4
...
def run_one(model, i):
    out_stats = STATS / f"steam_L4_{model}_snap{i}.npz"
    if out_stats.exists():
        print(f"steam_L4 {model} snap{i} already done, skipping")
        return
    src = np.load(STATS / f"{model}_snap{i}.npz")
    h_profile = src["h_profile"]
    qt_profile = src["qt_profile"]
    RUNS.mkdir(exist_ok=True)
    out_nc = RUNS / f"steam_L4_{model}_snap{i}.nc"
    ...
    print(f"steam_L4 {model} snap{i} done", flush=True)
