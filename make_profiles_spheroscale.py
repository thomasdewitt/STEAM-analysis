...
Same layout family as make_deltas.py. Two figures:
  figs/profiles_spheroscale.png — raw profiles: host (solid), STEAM 96 km (dashed),
    STEAM ls=10 m const (dotted), per model color.
  figs/deltas_spheroscale.png — STEAM-minus-host deltas for both outer scales on
    the same inter-LES pairwise spread band (96 km thin, ls=10 m const thick).
Requires stats/steam_96_* and steam_ls10_* from run_steam_96.py / run_steam_ls10.py.
"""
...
steamLs10 = {m: snapshot_mean(f"steam_ls10_{m}") for m in models}
...
axes[2].legend(fontsize=6.5, loc="upper right",
               title="solid host / dash 100-1 m / dot 10 m const")
fig.suptitle("Spheroscale at 96 km: linear 100->1 m vs constant 10 m (same seeds, frozen config)",
             fontsize=10)
fig.tight_layout()
(HERE / "figs").mkdir(exist_ok=True)
fig.savefig(HERE / "figs" / "profiles_spheroscale.png")
plt.close(fig)
...
axes[0].set_ylabel("z [km]")
axes[2].legend(fontsize=6.5, loc="upper right",
               title="thin 100-1 m / thick 10 m const")
fig.suptitle("STEAM $-$ host LES: spheroscale linear vs constant 10 m", fontsize=10)
fig.tight_layout()
fig.savefig(HERE / "figs" / "deltas_spheroscale.png")
plt.close(fig)

print("wrote figs/profiles_spheroscale.png, figs/deltas_spheroscale.png")
