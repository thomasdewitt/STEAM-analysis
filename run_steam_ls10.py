#!/usr/bin/env python3
"""run_steam_96.py variant: constant 10 m spheroscale (vs linear 100 m -> 1 m).
...
2026-07-27. Writes runs/steam_ls10_<model>_snap<i>.nc and
stats/steam_ls10_<model>_snap<i>.npz. Skips completed snapshots (restartable).

Usage: python run_steam_ls10.py [model ...]   (default: all with extracted stats)
"""
...
def run_one(model, i):
    out_stats = STATS / f"steam_ls10_{model}_snap{i}.npz"
    if out_stats.exists():
        print(f"steam_ls10 {model} snap{i} already done, skipping")
        return
    src = np.load(STATS / f"{model}_snap{i}.npz")
    h_profile = src["h_profile"]
    qt_profile = src["qt_profile"]
    RUNS.mkdir(exist_ok=True)
    out_nc = RUNS / f"steam_ls10_{model}_snap{i}.nc"
    ...
    print(f"steam_ls10 {model} snap{i} done", flush=True)
